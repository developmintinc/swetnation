...
namespace MerchantTribeStore
{
    public partial class TestImageUpload : BaseProductAdminPage
    {
        ...
        public void ShrinkImageFileOnUpload(string originalFile, string newFileName, int maxWidth, int maxHeight, FileUpload file)
        {
        ...
        protected void UploadButton_Click(object sender, EventArgs e)
        {
            ...
            if (tryThisInstead && this.FileUploadControl.HasFile)
            {
                ...
                if (MerchantTribe.Commerce.Storage.DiskStorage.ValidateImageType(ext))
                {
                    ...
                    if ((MerchantTribe.Commerce.Storage.DiskStorage.UploadProductImage(MTApp.CurrentStore.Id, p.Bvin, this.FileUploadControl.PostedFile)))
                    {
                ...
                try
                {
                    ...
                    if (FileUploadControl.PostedFile.ContentType == "image/jpeg" || FileUploadControl.PostedFile.ContentType == "image/png" || FileUploadControl.PostedFile.ContentType == "image/gif")
                    {
                        if (FileUploadControl.PostedFile.ContentLength < 102400)
                        {
                            ...
                            filename = Server.MapPath("~/Images/sites/1/products/7c57ac98-b639-4bf4-8ba5-a63d0bfbcfc3/") + filename;
                            ...
                            {
                                EventLog.LogEvent("TestImageUpload", "Directory does not exists and I am going to create it", EventLogSeverity.Debug);
                                Directory.CreateDirectory(Path.GetDirectoryName(filename));
                            }
                            else
                            {
                                EventLog.LogEvent("TestImageUpload", "Directory exists", EventLogSeverity.Debug);
                            }

                            // Save uploaded file
                            EventLog.LogEvent("TestImageUpload", "Saving file", EventLogSeverity.Debug);
                            FileUploadControl.PostedFile.SaveAs(filename);
                            ...
                            ShrinkImageFileOnUpload(filename, outputFile, 240, 240, FileUploadControl);
